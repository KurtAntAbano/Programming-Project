...
class studentProject():
    ...
    def showAllRecords(self):
        try:
            ...
            cursor = conn.execute(''' SELECT StudentName, SongContents FROM  SONG_DATABASE_V1 ''')
    ...
    def give_rows(self):
        ...
        # Select all records in a table:
        cursor = conn.cursor()
        cursor.execute(''' SELECT StudentID, StudentName, SongName, SongContents FROM SONG_DATABASE_V1 ''')
    ...
    def searchUser(self, givenUser, givenpassword):
        conn = sqlite3.connect('Student_songs_V1.db')
        # Select all records in a table:
        cursor = conn.execute(''' SELECT Email, Password FROM  SONG_DATABASE_V1 ''')
        for row in cursor:
