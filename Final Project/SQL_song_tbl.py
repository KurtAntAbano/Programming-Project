...
# This module/file shows code for the song table
# _________________________________________________________________________________#
...
class studentProject():
    ...
    def deleteRecord(self, givensongname):
        try:
            conn = sqlite3.connect('Student_songs.db')
            conn.execute("DELETE FROM SONG_DATABASE WHERE  SongName =?", (givensongname,))
            print("deleted")
            conn.commit()
            conn.close()
            return True
...
if __name__=="__main__":
    ...
    student_DB.deleteRecord("filler song 3")